​Task 7:-Given an array A of size N (at least three), you need to find its first, second and third maximum elements.
Input:-
The first line of the input contains the number of test cases T.
For each test case, the first line of the input contains an integer N denoting the number of elements in the array A. The next line contains N (space separated) elements of A.

Sample Input:
3
5
1 4 2 4 5
6
1 3 5 7 9 8
7
11 22 33 44 55 66 77

Sample Output:
5 4 4
9 8 7
77 66 55

import java.io.*; // for handling input/output
import java.util.*; // contains Collections framework

class Main {

    public static void main (String[] args) {

                      Scanner sc=new Scanner(System.in);
                      int a=sc.nextInt();
                      while (a>0){
                          int b=sc.nextInt();

                          int arr[]=new int [b];

                          for(int i=0; i<b; i++){

                              arr[i]=sc.nextInt();
                          }

                          Arrays.sort(arr);
                          System.out.println(arr[b-1] + " " + arr[b-2] + " " + arr[b-3]);
                          a--;

                      }

    }

}
